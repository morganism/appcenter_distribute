...
module AppCenter
  ...
  class MetadataUploader < BaseAPI
    ...
    def initialize(response)
      super(response.o_n, response.app_name, response.api_token)
      @response = response
    end

    def upload_metadata(file_name, content_type)
      file_size_bytes = `#{FILE_SIZE_COMMAND}`.strip
      metadata_url = URI("#{BASE_URL}#{@o_n}/#{@app_name}/uploads/releases/#{@response.id}")
  ...
  class StatusChecker < BaseAPI
    def check_status
      status_url = URI("#{BASE_URL}#{@o_n}/#{@app_name}/uploads/releases/#{@response.id}")
  ...
  class ReleaseDistributor < BaseAPI
    def distribute_release(d_g)
      distribute_url = URI("#{BASE_URL}#{@o_n}/#{@app_name}/releases/#{@response.release_id}")
      request = Net::HTTP::Patch.new(distribute_url)
      request.body = { destinations: [{ name: d_g }] }.to_json
      request['Content-Type'] = 'application/json'
      request['Accept'] = 'application/json'
      request['X-API-Token'] = @api_token
